...
def test_build_options_permission_mode_and_resume(monkeypatch, tmp_path):
    monkeypatch.setattr(settings, "chat_permission_mode", "bypassPermissions")
    monkeypatch.setattr(settings, "anthropic_base_url", "http://127.0.0.1:8088")
    monkeypatch.setattr(settings, "chat_default_model", "grok-4.5")
...
def test_build_options_drops_stale_resume_when_transcript_missing(monkeypatch, tmp_path):
    monkeypatch.setattr(settings, "chat_permission_mode", "bypassPermissions")
    monkeypatch.setattr(settings, "anthropic_base_url", "http://127.0.0.1:8088")
    ...
    session["sdk_session_id"] = "missing-resume-id"
    # No Claude transcript under this cwd → resume must be omitted.
    opts = build_options(session)
    assert opts.resume is None
    assert get_session(session["id"])["sdk_session_id"] is None


def test_build_options_keeps_resume_when_transcript_exists(monkeypatch, tmp_path):
    monkeypatch.setattr(settings, "chat_permission_mode", "bypassPermissions")
    monkeypatch.setattr(settings, "anthropic_base_url", "http://127.0.0.1:8088")
...
def test_is_missing_resume_session_error_detects_cli_messages():
    assert agent_bridge.is_missing_resume_session_error(
    ...
    assert not agent_bridge.is_missing_resume_session_error(
